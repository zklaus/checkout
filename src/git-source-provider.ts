...
export async function getSource(settings: IGitSourceSettings): Promise<void> {
  ...
  try {
    ...
    if (settings.fetchDepth <= 0) {
      let refSpec: string[] = (settings.singleBranch ? refHelper.getRefSpec(settings.ref, settings.commit) : refHelper.getRefSpecForAllHistory(settings.ref, settings.commit))
    ...
    if (settings.submodules) {
      ...
      await git.submoduleUpdate(settings.fetchDepth, settings.nestedSubmodules, settings.submodulesFilter)
